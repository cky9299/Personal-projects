...
uint public winnerIndex;
    ...
    function enter() public payable {
        //solidity dosent allow us to return only entrypriceinwei
        (uint entryPriceInWei,uint priceUint, uint decimals) = getEntranceFee();
        (uint lowerBound,uint upperBound) = calBounds();
        require(lottery_state==LOTTERY_STATE.OPEN,'be patient. awaiting cleavage');
    ...
    uint priceUint = uint(price);
        //uint ethEntryFee = uint(price)*10**10*
    ...
    function withdrawLink() public payable onlyOwner {
        payable(owner).transfer(address(this).balance);
        //transfer funds from this contract to the owner's wallet
